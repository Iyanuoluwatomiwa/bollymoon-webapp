...
export const googleSignIn = async () => {
  const { error } = await supabase.auth.signInWithOAuth({
    provider: 'google',
    options: {
      redirectTo: 'https://lsznjxjqhqhftiquvoxl.supabase.co/auth/v1/callback',
    },
  })
  if (error) {
    toast.error(error.message)
...
export const completeRegistration = async (props: any) => {
  ...
  if (role === 'buyer') {
    ...
    if (buyerError) {
      toast.error(buyerError.message)
      setSubmitting(false)
      return
    }
  }
  ...
  if (role === 'logistics') {
    ...
    const { error: logisticsError } = await supabase.from('logistics').insert({
      id: user?.id,
      firstname,
      lastname,
      businessname,
    ...
    if (logisticsError) {
      toast.error(logisticsError.message)
      setSubmitting(false)
      return
    }
  }
