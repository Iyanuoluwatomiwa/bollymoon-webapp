      ...
      <Container className="py-8 sm:py-10 bg-white">
        <div className="space-y-8 md:space-y-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 sm:gap-2 lg:gap-4 max-w-5xl mx-auto">
            {shopByCategories.map(({ title, image, url }) => {
              return (
                <Link to={`/shop?category=${url}`} key={title}>
                  <div className=" rounded-sm pb-3 sm:pb-4 space-y-3 sm:space-y-4 group cursor-pointer shadow-sm hover:shadow-md overflow-hidden">
