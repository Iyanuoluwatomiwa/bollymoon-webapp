    ...
    <Card className="mt-4">
      ...
      <CardContent>
        {variants.map((variant) => (
          <div
            key={variant.size}
            className="font-medium border py-4 px-2 rounded-lg mb-4 md:space-y-0 space-y-2  "
          >
            <p className="flex items-center text-base gap-1">
              <Label>Size:</Label> {variant.size}
            </p>
            <ul className="space-y-4">
              {variant.colors.map(({ color, quantity }, index) => (
                <li key={index}>
                  <div className="flex flex-row items-center justify-between gap-4 pl-2">
